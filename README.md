...
| Template | What it is |
|---|---|
| **Test Runner** — `cirunner*` | Self-contained Gitea Actions runner. Runs entirely as the unprivileged AMP user — no root, no container runtime — and reaches a tailnet-only Gitea through userspace Tailscale. [Setup](cirunnerREADME.md) |
...
`Test Runner` and `k3s Node` are two answers to the same problem — putting CI on a
remote AMP host. Which one applies depends entirely on whether you have root
there. `Test Runner` does not need it.
...
**Do not rename this repository.** The `Test Runner` and `k3s Node` update stages
